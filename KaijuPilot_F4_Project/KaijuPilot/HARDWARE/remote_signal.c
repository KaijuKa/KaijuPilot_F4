...
void TIM1_CH1_Input_Init(u32 pre,u16 psc)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA,ENABLE); //初始化结构体创建及时钟初始化
...
void PPM_calcu(u32 Pulselength)
{
	...
	{                                   //处于合理通道值内 处理 否则遗弃
		if(Pulselength > PULSE_MIN && Pulselength < PULSE_MAX)
		{
			...
			ch_watch_dog_feed(channel_num); //喂狗
			channel_num++;
		}
	}
}
...
void ch_watch_dog_feed(u8 ch_n)       //喂狗
{
	ch_offline_time[ch_n] = 0;
}

/*******************************************************************************
* 函 数 名         : ch_offline_check
* 函数功能		     : ppm 掉线检测
* 输    入         : 周期
* 输    出         : 无
*******************************************************************************/
void ch_offline_check(u8 dT_ms)       //通道值丢失检测
{
	u8 i;
	for(i = 0;i<CH_NUM;i++)
	{
